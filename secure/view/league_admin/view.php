...
if (Session::is_administrator()) {
    ...
    print "<tr><th class='id'>Id</th><th class='club'>Name</th><th class='address'>Address</th><th class='button last'></th></tr>";
    foreach ($leagueData->club_list as $club) {
        print_form(
            array('id', 'club', 'address'), array($club->id, $club->name, $club->address),
            array('club_id'), array($club->id)
        );
    }
    print_create_form_start('club');
    print "<tr class='create_row'><td colspan='2' class='club last'><input class='show_validation' name='name' type='text' pattern='.{3,25}' required='required'></td><td class='address last'><input name='address' type='text' pattern='.{10,125}'></td><td class='button last'><input type='submit' name='create' value='create'></td></tr>";
    print_form_table_end();

    // LEAGUES
    print_table_start('Leagues', 'action_table');
    print "<tr><th class='id'>Id</th><th class='club'>Club</th><th class='league'>League</th><th class='button last'></th></tr>";
    foreach ($leagueData->league_list as $league) {
        print_form(
            array('id', 'club', 'league'), array($league->id, $leagueData->print_club_name($league->club_id), $league->name),
            array('league_id'), array($league->id)
        );
    }
    print_create_form_start('league');
    print "<tr class='create_row'><td colspan='2' class='club last'>";
    ...
    print "<tr><th class='id'>Id</th><th class='league'>League</th><th class='division'>Division</th><th class='button last'></th></tr>";
    foreach ($leagueData->division_list as $division) {
        print_form(
            array('id', 'league', 'division'), array($division->id, $leagueData->print_league_name($division->league_id), $division->name),
            array('division_id'), array($division->id)
        );
    }
    print_create_form_start('division');
    print "<tr class='create_row'><td colspan='2' class='league last'>";
    ...
    print "<tr><th class='id'>Id</th><th class='division'>Division</th><th class='status'>Status</th><th class='date'>Start</th><th class='date'>End</th><th class='button last'></th></tr>";
    foreach ($leagueData->round_list as $round) {
        print_form(
            array('id', 'division', 'status', 'date', 'date'), array($round->id, $leagueData->print_division_name($round->division_id), $round->status, date('d-M-Y', $round->start), date('d-M-Y', $round->end)),
    ...
    if (count($leagues) > 0) {
        ...
        print "<tr class='create_row'><td colspan='2' class='division last' colspan='2'>";
        print "<select name='league_id'>";
    ...
    print "<tr><th class='id'>Id</th><th class='division'>Division</th><th class='name'>User</th><th class='button last'></th></tr>";
    foreach ($leagueData->player_list as $player) {
        print_form(
            array('id', 'division', 'name'), array($player->id, $leagueData->print_division_name($player->division_id), $leagueData->print_user_name($player->id, false)),
            array('player_id'), array($player->id)
        );
    }
    print_create_form_start('player');
    print "<tr class='create_row'><td colspan='2' class='division last'>";
    ...
    foreach ($leagueData->round_list as $round_table) {
        ...
        print "<tr><th class='id'>Id</th><th class='round'>Round</th><th class='player'>Player One</th><th class='player'>Player Two</th><th class='button last'></th></tr>";
        foreach ($leagueData->match_list as $match) {
            if ($match->round_id == $round_table->id) {
                print_form(
                    array('id', 'round', 'player', 'player'), array($match->id, $leagueData->print_round_name($match->round_id, false), $leagueData->print_user_name($match->player_one_id, false), $leagueData->print_user_name($match->player_two_id, false)),
                    array('match_id'), array($match->id)
                );
            }
        }
        print_create_form_start('match');
        print "<tr><td colspan='2' class='round last'>";
