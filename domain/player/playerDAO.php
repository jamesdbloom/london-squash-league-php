...
/* --USERS WHO HAVE NEVER SET A SCORE FOR A MATCH--

select distinct USER.NAME FROM MATCHES JOIN PLAYER ON MATCHES.PLAYER_ONE_ID = PLAYER.PLAYER_ID OR MATCHES.PLAYER_TWO_ID = PLAYER.PLAYER_ID JOIN USER ON PLAYER.USER_ID = USER.USER_ID where PLAYER_ID NOT IN (select distinct PLAYER.PLAYER_ID from MATCHES JOIN PLAYER ON MATCHES.PLAYER_ONE_ID = PLAYER.PLAYER_ID OR MATCHES.PLAYER_TWO_ID = PLAYER.PLAYER_ID JOIN USER ON PLAYER.USER_ID = USER.USER_ID where SCORE IS NOT NULL ORDER BY NAME);

create table INACTIVE_PLAYERS(PLAYER_ID INT NOT NULL);

insert into INACTIVE_PLAYERS (select distinct PLAYER.PLAYER_ID FROM MATCHES JOIN PLAYER ON MATCHES.PLAYER_ONE_ID = PLAYER.PLAYER_ID OR MATCHES.PLAYER_TWO_ID = PLAYER.PLAYER_ID JOIN USER ON PLAYER.USER_ID = USER.USER_ID where PLAYER_ID NOT IN (select distinct PLAYER.PLAYER_ID from MATCHES JOIN PLAYER ON MATCHES.PLAYER_ONE_ID = PLAYER.PLAYER_ID OR MATCHES.PLAYER_TWO_ID = PLAYER.PLAYER_ID JOIN USER ON PLAYER.USER_ID = USER.USER_ID where SCORE IS NOT NULL ORDER BY NAME));

UPDATE PLAYER SET STATUS='inactive' WHERE PLAYER_ID IN (select PLAYER_ID from INACTIVE_PLAYERS);
...
//      $league_data->create_matches(Parameters::read_post_input('ignore_round_status'));
    ...
    public static function update_status_by_id($player_id, $status)
    {
        $query =
            "UPDATE " . self::table_name .
                " SET " . self::status_column . " = :" . self::status_column .
        ...
        $parameters = array(
            ':' . self::status_column => $status,
            ':' . self::id_column => $player_id,
        );
        self::insert_update_delete_create($query, $parameters, 'update player status by id ');
    }
